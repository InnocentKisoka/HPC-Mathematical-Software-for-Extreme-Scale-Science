...
def ComputeRHS(nx, ny, f_value):
    ...
    # Since f is constant, we fill it with f_value.
    rhs = np.full((nx * ny,), f_value)
    # Apply Dirichlet boundary conditions (u = 0 on all boundaries).
    # Set boundary values in the RHS to account for Dirichlet conditions.
    rhs[:nx] = 0             # Bottom boundary (y=0)
    rhs[-nx:] = 0            # Top boundary (y=1)
    rhs[::nx] = 0            # Left boundary (x=0)
    rhs[nx-1::nx] = 0        # Right boundary (x=1)
...
def ComputeMatrix(nx, ny, dx, dy):
    ...
    for j in range(ny):
        for i in range(nx):
            ...
            # Central difference coefficients for Laplacian
            data.append(-4)  # Main diagonal
