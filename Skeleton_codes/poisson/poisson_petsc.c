...
PetscErrorCode ComputeRHS(KSP ksp, Vec b, void *ctx) {
  ...
  for (PetscInt j = info.ys; j < info.ys + info.ym; ++j) {
    for (PetscInt i = info.xs; i < info.xs + info.xm; ++i) {
      array[j][i] = user->c * hx * hy;  // Multiply forcing term by cell area
    }
  }
  ...
  for (PetscInt j = info.ys; j < info.ys + info.ym; ++j) {
    for (PetscInt i = info.xs; i < info.xs + info.xm; ++i) {
      ...
      if (i == 0 || i == info.mx - 1 || j == 0 || j == info.my - 1) {
        ...
        col[0].i = i; col[0].j = j - 1; v[0] = hy2;    // Bottom
        col[1].i = i - 1; col[1].j = j; v[1] = hx2;    // Left
        col[2].i = i; col[2].j = j;     v[2] = -2.0 * (hx2 + hy2); // Center
        col[3].i = i + 1; col[3].j = j; v[3] = hx2;    // Right
        col[4].i = i; col[4].j = j + 1; v[4] = hy2;    // Top
